...
public class ProfileIOHandler {
    ...
    public int[] getFollowersAndFollowing() throws SQLException {
        Connection connection = DataBaseConnection.getConnection();
        String queryFollowers = "SELECT COUNT(*) AS follows_count FROM Follows WHERE following_id = ?";
        String queryFollowing = "SELECT COUNT(*) AS  FROM Follows WHERE follower_id = ?";
        ...
        if (resultSetFollowers.next()) {
            followersCount = resultSetFollowers.getInt("follower_id");
        }

        if (resultSetFollowing.next()) {
            followingCount = resultSetFollowing.getInt("following_id");
        }
